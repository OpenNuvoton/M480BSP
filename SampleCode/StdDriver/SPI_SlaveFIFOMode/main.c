 ...
 * @file     main.c
 * @version  V3.0
 * @brief    Configure QSPI0 as Slave mode and demonstrate how to communicate
...
void SPI_Init(void)
{
    ...
    SPI_Open(QSPI0, SPI_SLAVE, SPI_MODE_0, 32, (uint32_t)NULL);
}
...
int main(void)
{
    ...
    while(u32RxDataCount < TEST_COUNT) {
        ...
        if((SPI_GET_TX_FIFO_FULL_FLAG(QSPI0) == 0) && (u32TxDataCount < TEST_COUNT))
            SPI_WRITE_TX(QSPI0, g_au32SourceData[u32TxDataCount++]); /* Write to TX FIFO */
        /* Check RX EMPTY flag */
        if(SPI_GET_RX_FIFO_EMPTY_FLAG(QSPI0) == 0)
            g_au32DestinationData[u32RxDataCount++] = SPI_READ_RX(QSPI0); /* Read RX FIFO */
    }
