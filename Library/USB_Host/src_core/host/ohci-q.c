...
static void periodic_link (struct ohci_hcd *ohci, struct ed *ed)
{
    ...
    USB_vdebug("link %sed %p branch %d [%dus.], interval %d\n",
...
static int ed_schedule (struct ohci_hcd *ohci, struct ed *ed)
{
    ...
    default:
        ...
        if (branch < 0) {
            USB_debug("ERR %d, interval %d msecs, load %d\n",
...
static void periodic_unlink (struct ohci_hcd *ohci, struct ed *ed)
{
    ...
    USB_vdebug("unlink %sed %p branch %d [%dus.], interval %d\n",
               (ed->hwINFO & cpu_to_hc32 (ohci, ED_ISO)) ? "iso " : "",
               ed, ed->branch, ed->load, ed->interval);
}
...
static int td_done(struct ohci_hcd *ohci, struct urb *urb, struct td *td)
{
    ...
    if (tdINFO & TD_ISO) {
        ...
        if (cc != TD_CC_NOERROR)
            USB_vdebug("urb %p iso td %p (%d) len %d cc %d\n",
        ...
        if (cc != TD_CC_NOERROR && cc < 0x0E)
            USB_vdebug("urb %p td %p (%d) cc %d, len=%d/%d\n",
...
static void ed_halted(struct ohci_hcd *ohci, struct td *td, int cc)
{
    ...
    ed = td->ed;
    tmp = td->td_list.next;
    toggle = ed->hwHeadP & cpu_to_hc32 (ohci, ED_C);
    ...
    /* fallthrough */
    default:
        USB_debug("urb %p path %s ep%d%s %08x cc %d --> status %d\n",
                  urb, urb->dev->devpath,
...
static struct td *dl_reverse_done_list (struct ohci_hcd *ohci)
{
    ...
    printf("DQ => ");
    for (i = 0; td_dma && i < 6; i++) {
        printf("0x%x => ", td_dma);
        td_dma = ((struct td    *)(td_dma & ~0x3))->hwNextTD;
    }
    printf(" |\n");
#endif
