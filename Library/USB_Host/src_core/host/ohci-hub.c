...
#define dbg_port(hc,label,num,value) \
    USB_vdebug("%s roothub.portstatus [%d] = 0x%08x%s%s%s%s%s%s%s%s%s%s%s%s\n", \
        label, num, temp, (temp & RH_PS_PRSC) ? " PRSC" : "", \
        ...
        (temp & RH_PS_PSS) ? " PSS" : "", \
        (temp & RH_PS_PES) ? " PES" : "", (temp & RH_PS_CCS) ? " CCS" : "");
...
static int ohci_hub_status_data (struct usb_hcd *hcd, char *buf)
{
    ...
    if ((ohci->flags & OHCI_QUIRK_AMD756)
        ...
        USB_error("bogus NDP, rereads as NDP=%d\n",
...
)
{
    ...
    rh = roothub_b (ohci);
    memset((u8 *)&(desc->u.hs.DeviceRemovable[0]), 0xff, sizeof(desc->u.hs.DeviceRemovable));
...
{
    __hc32 __iomem *portstat;
    u32     temp = 0;
    u16     now;
    u16     reset_done;
    int     limit_1;

    portstat = &ohci->regs->roothub.portstatus [port];
    now = ohci_readl(ohci, &ohci->regs->fmnumber);
    reset_done = now + PORT_RESET_MSEC;
    limit_1 = DIV_ROUND_UP(PORT_RESET_MSEC, PORT_RESET_HW_MSEC);
    ...
    do {
         ...
         */
        if (limit_2 < 0) {
            USB_debug("port[%d] reset timeout, stat %08x\n", port, temp);
            break;
        }
