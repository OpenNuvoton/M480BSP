 ...
 */
#define OHCI_INTR_SO    (1)         /* scheduling overrun */
...
#define OHCI_INTR_MIE   0x80000000  // (1 << 31)    /* master interrupt enable */
...
#define RH_A_POTPGT 0xff000000      // (0xff << 24)     /* power on to power good time */
...
typedef struct urb_priv {
    ...
    struct td   *td[32];    // *td[0];  // all TDs in this request
...
{
#if 1   // NUvoton ported
    // hcd_priv is actually locate behind hcd, see usb_create_shared_hcd
    return (struct usb_hcd *)((__u32)ohci - sizeof(struct usb_hcd));
#else
...
#define cpu_to_hc16(ohci, x)        (x)
#define cpu_to_hc16p(ohci, x)       (*(u16 *)(x))
#define cpu_to_hc32(ohci, x)        (x)
#define cpu_to_hc32p(ohci, x)       (*(u32 *)(x))
#define hc16_to_cpu(ohci, x)        (x)
#define hc16_to_cpup(ohci, x)       (*(u16 *)(x))
#define hc32_to_cpu(ohci, x)        (x)
#define hc32_to_cpup(ohci, x)       (*(u32 *)(x))
#else
...
static inline u32 roothub_a (struct ohci_hcd *hc)
{
    ...
    return ohci_readl(hc, &hc->regs->roothub.a);
}
...
static inline u32 roothub_portstatus (struct ohci_hcd *hc, int i)
{
    ...
    return ohci_readl(hc, &hc->regs->roothub.portstatus[i]);
}
